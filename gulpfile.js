...
gulp.task('img', function () {
	return gulp.src(watch.img).pipe(gulp.dest(dist.img));
});

// Шрифти просто копіюємо
gulp.task('fonts', function () {
	return gulp.src(watch.fonts).pipe(gulp.dest(dist.fonts));
});

// Бібліотеки просто копіюємо
gulp.task('libs', function () {
	return gulp.src(watch.libs).pipe(gulp.dest(dist.libs));
});
...
gulp.task('build', function (cb) {
	runSequence('clean', 'fonts', 'img', 'libs', 'minJs', 'sass', 'sassMin', 'html', cb);
});
